...
// For Api header
$API_NAME = 'FL20241106183135632FL20241106183136356FL20241106183137344';
...
if ($get_helper['helper_name'] == "app_details") {
    ...
    while ($data = mysqli_fetch_assoc($result)) {
        ...
        // Remove Envato API Key (this line is eliminated)
        ...
        $data_arr['is_xui_vodextra'] = $data['is_xui_vodextra'];
        $data_arr['is_xui_vod3rparty'] = $data['is_xui_vod3rparty'];
        ...
        $data_arr['tmdb_key'] = $data['account_delete_intruction'];
         $data_arr['RABBITSTREAM_key'] = $data['RABBITSTREAM_SOURCE_API'];
          $data_arr['vod_extra_pakage_name_key'] = $data['vod_extra_pakage_name'];
           $data_arr['vod_extra_link_key'] = $data['vod_extra_link'];
           //app name and logo
            $data_arr['app_name'] = $data['app_name'];
             $data_arr['app_logo'] = $data['app_logo'];
...
if (!isset($_POST['val']) || empty($_POST['data'])) {
    header('Content-Type: application/json; charset=utf-8');

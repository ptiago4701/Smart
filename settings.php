<?php $page_title="Settings Admin";
    ...
    if ($_FILES['app_logo']['name'] != "") {
        ...
        $timestamp = time();
        $app_logo = "app_logo_" . $timestamp . ".png";  // Adjust the extension as necessary
        $tpath1 = 'images/' . $app_logo;

        // Delete the existing image if it exists
        $old_logo_path = 'images/' . $img_row['app_logo'];
        ...
        if ($ext != 'png') {
            // Compress and save if it's not a PNG
            $pic1 = compress_image($_FILES["app_logo"]["tmp_name"], $tpath1, 80);
        } else {
            // Move the file directly if it's a PNG
            $tmp = $_FILES['app_logo']['tmp_name'];
            move_uploaded_file($tmp, $tpath1);
        }

    } else {
        // If no new image is uploaded, retain the existing app logo
        $app_logo = $img_row['app_logo'];
    }

    // Store the updated data in your SQL database
    $app_name = $_POST['app_name'];
    $update_query = "UPDATE tbl_settings SET app_name = '$app_name', app_logo = '$app_logo' WHERE id = '1'";
                                    ...
                                    <div class="mb-3 row">
                                        ...
                                        <div class="col-sm-10">
                                            <div class="row">
                                                ...
                                                <div class="col-md-3">
    <?php 
    $app_logo_path = 'images/' . $settings_data['app_logo'];
    
    if ($settings_data['app_logo'] != '' && file_exists($app_logo_path)) { ?>
        <div class="fileupload_img" id="imagePreview">
            <img type="image" src="<?= $app_logo_path ?>" style="width: 50px; height: 50px" alt="image" />
        </div>
    <?php } else { ?>
        <div class="fileupload_img" id="imagePreview">
